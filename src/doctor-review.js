...
function hasSelfCareSafetyInstruction(text) {
  return /不要自行|不建議自行|不能.*自行|不要先自行|下一針不要|不要.*照打|不要.*調劑量|不要.*停藥|不建議使用朋友剩下|不建議自己|吃剩下的抗生素|不能只靠(?:線上)?(?:文字|訊息)(?:判斷|診斷|開藥)|只靠(?:線上)?(?:文字|訊息)不能(?:診斷|開藥)|不能只靠.*(?:單一)?報告|不能.*直接安排手術|保證改善受孕|不能在線上告訴你最低劑量|不能在線上提供.*劑量|是否適合.*不能只靠訊息判斷|不能用來治療|不能取代檢查|不是保證|不等於一定是.*癌|PSA.*不能只靠訊息判斷|檢查報告需要醫師/.test(text);
}
...
function isShortSafetyContext(text) {
  return (
    ...
    (text.length <= 120 && /備孕|精液|精索靜脈曲張|陰囊超音波|心臟病|硝化甘油|陰莖水泡|破皮潰瘍|PSA/.test(text))
  );
}
