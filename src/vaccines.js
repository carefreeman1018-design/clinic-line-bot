...
export function answerVaccineQuestion(message) {
  ...
  if (asksPersonalSuitability(message)) {
    parts.push("有過敏史、懷孕/備孕、慢性病或正在用藥時，LINE 不能直接判斷能不能打，需由醫師或診所人員依個人狀況評估。");
...
function asksPriceOrStock(message) {
  return /價錢|價格|費用|多少錢|幾多錢|庫存|有貨|現貨|名額|預約時段|可預約/.test(message);
}

function asksPersonalSuitability(message) {
  return /過敏|懷孕|備孕|慢性病|免疫|吃藥|用藥|藥物|適合|能不能|可不可以|可以直接打|副作用|禁忌/.test(message);
}
